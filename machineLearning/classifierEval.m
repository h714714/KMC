function [computedClass, logLike, recogRate, hitIndex, nnIndex]=classifierEval(classifier, DS, cPrm, plotOpt)
...
nnIndex=[];	% This is only available when the classifier is knnc.
...
%		[nearestDist, nnIndex] = sort(distMat, 1);
%		knncMat=reshape(cPrm.output(nnIndex(1:k,:)), k, testNum);	% knncMat(i,j) = class of i-th nearest point of j-th test input vector (size = k by testNum.)
		% Better replacement for the previous 3 lines
		nnIndex=knnMex(DS.input, cPrm.input, 2, k);
		knncMat=cPrm.output(nnIndex);	% See the above comments about knncMat
		% === For-loop version to avoid the above memory hog
	%	knncMat=zeros(k, testNum);
	%	for i=1:testNum
	%		distance=distSqrPairwise(cPrm.input, DS.input(:,i));
	%		[minDist, nnIndex]=sort(distance);
	%		knncMat(:,i)=cPrm.output(nnIndex(1:k));
	%	end
	...
	%	[junk, computedClass]=max(classCount, [], 1);
		...
		% === logLike
		if nargout>1
		% Vectorized but memory-inefficient
		%	distMat = distSqrPairwise(cPrm.input, DS.input);	% Squared Euclidean distance matrix between sampleInput and testInput ==> This is a memory hog!
		...
		% Memory-efficient for-loop version of doing the same thing as the above lines
			for i=1:classNum
				index=find(cPrm.output==i);
				for j=1:testNum
					if ~isempty(index)	% Normal situation
						distVec=distSqrPairwise(cPrm.input(:, index), DS.input(:,j));	% Distances of j-th test input to data in class i
						minDist=min(distVec);
						logLike(i,j)=1/(1+minDist);
					else	% No such class. This happens when doing LOO and there is only one instance for a class
						logLike(i,j)=nan;
					end
				end
			end
	...
	case 'svmc'
		addpath([mltRoot, '/externalTool/libsvm-3.21/matlab']);		% Add the path of libsvm by Prof. Chih-Jen Lin
	...
	case 'lsec'
		...
		b=DS.output';
		computedClass=round(A*cPrm);
	%	cPrm=A\b;
		logLike=[];
		recogRate=1-sum((A*cPrm-b).^2)/sum((b-mean(b)).^2);		% This is actually R-square for linear regression
		hitIndex=[];
	otherwise
